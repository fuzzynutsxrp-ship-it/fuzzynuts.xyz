(function() {
	...
	// ── Fuzzynuts Arcade Scoring Bridge ──
	if (!window._fuzzyArcade) {
		window._fuzzyArcade = {
  ...
  Player.prototype.setAnimation = function() {
	...
	Player.prototype.die = function () {
		...
		// ── Fuzzynuts Arcade: send score on death ──
		var fa = window._fuzzyArcade;
		fa.score = (this.coins * 200) + (fa.enemiesKilled * 500);
		try {
			window.parent.postMessage({
				type: 'fuzzynuts_arcade',
				game: 'mario',
				event: 'game_over',
				score: fa.score,
				sessionId: fa.sessionId,
				duration: Math.floor((Date.now() - fa.startTime) / 1000)
			}, '*');
		} catch(e) {}
	...
	Player.prototype.exit = function() {
		...
		// ── Fuzzynuts Arcade: send score on level complete (bonus!) ──
		var fa = window._fuzzyArcade;
		fa.score = (this.coins * 200) + (fa.enemiesKilled * 500) + 5000; // 5000 level complete bonus
		try {
			window.parent.postMessage({
				type: 'fuzzynuts_arcade',
				game: 'mario',
