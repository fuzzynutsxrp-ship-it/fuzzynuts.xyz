...
**Verdict:** ~42% of all documentation by size is frozen legacy handoff material. It is text-only (KB, not MB) so it is **not a disk/repo-size problem** — but it is a _navigation/signal_ problem and it contains the `GAME_SESSION_SECRET` and the (test) Xaman key. The two `_archive` subfolders both contain a near-identical `fuzzynuts_handoff.md` (line-for-line matches at line 260) — i.e. the archive itself has duplicates.
...
- **Verdict: NOT orphaned in code**, but its _runtime home_ (the VPS) is the teardown target in Directive 6. The package is named `rsc-account-server` and is a pnpm workspace member with a `start` script; it has its own `node_modules` (part of the 1.2 GB).
...
- Root config files (`turbo.json`, `tsconfig`, `.prettier*`, `eslint`) are legitimately root-level for a monorepo; no obvious config that _must_ move into a workspace.
...
4. **Stop billing:** after the snapshot completes, **Destroy** the droplet (Droplets → … → Destroy). _A powered-off droplet still bills_ — only Destroy stops compute charges (snapshot storage bills a small amount).
...
_End of Phase 2. No changes made. Awaiting your review before I draft Phase 3 (REORGANIZATION_PLAN.md, dry-run only)._
