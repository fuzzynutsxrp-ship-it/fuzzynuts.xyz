...
/* ─────────────────────────────────────────────────────────────
   Hero3DOverlay — Non-canvas HTML chrome that floats over the
   3D scene: logo, headline, CTAs, stats, holographic vault
   teaser, leaderboard snippet, scroll indicator.

   We keep this in DOM (not Drei <Html>) so it stays crisp,
   accessible, and fully styled by Tailwind.
   The overlay is pointer-events-none at the wrapper level and
   re-enables interaction on actual controls — this lets the
   3D canvas keep receiving pointer events for portals/clicks
   wherever the chrome isn't.
   ───────────────────────────────────────────────────────────── */
...
export function Hero3DOverlay() {
  const { isConnected, connect, isConnecting } = useWalletStore();

  return (
    <div className="absolute inset-0 z-[3] pointer-events-none">
      {/* Subtle dark gradient at top + bottom so HTML text remains readable
          over the 3D scene without hiding the artwork. */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: [
            "linear-gradient(to bottom, rgba(1,5,8,0.75) 0%, rgba(1,5,8,0.05) 22%, rgba(1,5,8,0.0) 55%, rgba(1,5,8,0.55) 88%, rgba(1,5,8,0.95) 100%)",
            "radial-gradient(ellipse 70% 45% at 50% 38%, rgba(251,191,36,0.06) 0%, transparent 70%)",
          ].join(", "),
        }}
      />

      <div className="relative h-full w-full container-main flex flex-col items-center justify-center pt-24 pb-10 text-center">
        {/* ── Logo ── */}
        <motion.div
        ...
        >
          <motion.div
            ...
            className="inline-block drop-shadow-[0_0_30px_rgba(245,196,66,0.45)]"
          >
            <Image
              ...
              width={160}
              height={107}
              className="w-24 h-auto sm:w-32 md:w-36 drop-shadow-[0_0_25px_rgba(245,196,66,0.35)]"
              priority
            />
          </motion.div>
        </motion.div>
        ...
        >
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[11px] sm:text-xs font-bold tracking-widest uppercase bg-[rgba(245,196,66,0.18)] backdrop-blur-sm text-[var(--color-gold)] border border-[rgba(251,191,36,0.25)]">
            🌰 Live on XRPL Mainnet · Immersive Mode
          </span>
        </motion.div>
        ...
        <motion.p
          ...
          className="text-lg sm:text-2xl md:text-3xl font-display font-semibold text-[var(--color-gold)] mb-2"
        >
          Play. Earn. Own.
        </motion.p>
        ...
        >
          The nuttiest meme coin on the XRP Ledger. Click a glowing portal to
          enter an arcade game — or scroll for the full lineup.
        </motion.p>
        ...
        >
          <div className="rounded-2xl px-5 py-4 sm:px-6 sm:py-5 bg-[rgba(1,5,8,0.55)] backdrop-blur-md border border-[rgba(251,191,36,0.25)] shadow-[0_10px_40px_rgba(0,0,0,0.45)] max-w-xl mx-auto">
