...
export class Game {
    constructor() {
        ...
        // Mobile: visualViewport fires when address bar shows/hides
    ...
    _resize() {
        ...
        // DESKTOP: maintain aspect ratio with small margin
        const vp = typeof window !== 'undefined' && window.visualViewport;
        const vpW = vp ? vp.width : window.innerWidth;
        const vpH = vp ? vp.height : window.innerHeight;
        const margin = 8;
        const w = Math.min(vpW - margin * 2, CONFIG.CANVAS_WIDTH);
        const h = Math.min(vpH - margin * 2, CONFIG.CANVAS_HEIGHT);
    ...
    gameOver() {
        ...
        // ── Fuzzynuts Arcade score bridge ──────────────────────────
        // Submit a composite POINTS score to the shared arcade leaderboard.
        // Formula: (kills × 10) + (survival seconds × 5) + (level × 100)
        // This makes every run metric contribute to the final score.
        this.arcadeScore = (this.kills * 10)
            + (Math.floor(this.gameTime) * 5)
            + (this.player.level * 100);
        if (typeof FuzzyScoreSubmit === 'function') {
            try {
                FuzzyScoreSubmit('survivors', this.arcadeScore, this.gameTime);
            } catch (e) {
                console.warn('[FuzzyScore] bridge error', e);
            }
        }

        // Populate arcade score display in the game-over card
        // (must run after showGameOver to guarantee the DOM is visible)
        const arcadeScoreEl = document.getElementById('arcadeScoreVal');
        if (arcadeScoreEl) {
            arcadeScoreEl.textContent = this.arcadeScore.toLocaleString();
        }

        this.ui.showGameOver(this);

        // Re-populate after showGameOver in case it touches the element
        if (arcadeScoreEl) {
            arcadeScoreEl.textContent = this.arcadeScore.toLocaleString();
        }
    }
