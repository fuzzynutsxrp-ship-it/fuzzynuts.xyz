...
export class Player {
    ...
    render(ctx) {
        ...
        grad.addColorStop(0, 'rgba(218,165,32,0.35)');
        ...
        ctx.beginPath();
        ctx.ellipse(this.x - this.size * 0.6, this.y - this.size * 0.8, this.size * 0.5, this.size * 0.9, -0.4, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc(this.x, this.y + this.size * 0.15, this.size * 0.55, 0, Math.PI * 2);
        ctx.fill();

        // Ears
        ctx.fillStyle = '#C87533';
        ctx.beginPath();
        ctx.arc(this.x - this.size * 0.55, this.y - this.size * 0.75, this.size * 0.28, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(this.x + this.size * 0.55, this.y - this.size * 0.75, this.size * 0.28, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.fillStyle = '#222';
        ctx.beginPath();
        ctx.arc(this.x - this.size * 0.28, this.y - this.size * 0.2, 2.5, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(this.x + this.size * 0.28, this.y - this.size * 0.2, 2.5, 0, Math.PI * 2);
        ctx.fill();

        // Garlic/Stink Cloud aura ring
        const garlic = this.weapons.find((w) => w.id === 'garlic');
        if (garlic) {
            const range = garlic.getRange(this);
            const t = performance.now() / 400;
            ctx.strokeStyle = `rgba(218,165,32,${0.25 + Math.sin(t) * 0.08})`;
            ctx.lineWidth = 2;
            ctx.beginPath();
...
export class ExpOrb {
    ...
    update(dt, game) {
        ...
        if (d < CONFIG.PICKUP_DISTANCE) {
            ...
            game.createFloatingText(`+${this.value}XP`, p.x, p.y - 40, '#DAA520');
            game.audio.pickup();
    ...
    render(ctx) {
        ...
        g.addColorStop(0, 'rgba(218,165,32,0.55)');
        ...
        ctx.beginPath();
        ctx.arc(this.x, this.y - this.size * 0.3, this.size * 0.7, Math.PI, Math.PI * 2);
        ctx.fill();
        ctx.restore();
    }
}
